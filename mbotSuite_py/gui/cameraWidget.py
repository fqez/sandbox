...
class CameraWidget(QtWidgets.QWidget):
    ...
    def __init__(self, winParent, cam=0):
    ...
    def initUI(self):
        ...
        if self.cam == 0:
            self.setWindowTitle("PiCamera")
        elif self.cam == 1:
            self.setWindowTitle("IR Left")
        elif self.cam == 2:
            self.setWindowTitle("IR Right")
    ...
    def updateImage(self):
        ...
        if self.cam == 0:
            c = self.winParent.getCamera()
        elif self.cam == 1:
            c = self.winParent.getIRL()
        elif self.cam == 2:
            c = self.winParent.getIRR()
        if (c):
            img = c.getImage().data
            if img is not None:
