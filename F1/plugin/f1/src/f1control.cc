...
void
F1Control::Load(ModelPtr model, sdf::ElementPtr _sdf){
...
void
F1Control::Init(F1Sensors *sensors){
    ...
    updateConnection = gazebo::event::Events::ConnectWorldUpdateBegin(
        boost::bind(&F1Control::OnUpdate, this, _1));
}

void
F1Control::OnUpdate(const gazebo::common::UpdateInfo & /*_info*/){

            pthread_mutex_lock(&mutex);

            robotMotors.targetRightSteerPos=robotMotors.w;//*0.58-this->steeringRightJoint->GetAngle(0).Radian();
            robotMotors.targetLeftSteerPos=robotMotors.w;//*0.58-this->steeringLeftJoint->GetAngle(0).Radian();

            this->motorspeed = robotMotors.v;
            ...
            this->steeringRightJoint->SetPosition(0, robotMotors.targetRightSteerPos);
            //this->steeringRightJoint->SetPosition(1, robotMotors.targetRightSteerPos);
            //this->steeringRightJoint->SetPosition(2, robotMotors.targetRightSteerPos);

            this->steeringLeftJoint->SetPosition(0, robotMotors.targetLeftSteerPos);
            //this->steeringLeftJoint->SetPosition(1, robotMotors.targetLeftSteerPos);
            //this->steeringLeftJoint->SetPosition(2, robotMotors.targetLeftSteerPos);
            pthread_mutex_unlock(&mutex);
}

void
F1Control::teleport(gazebo::math::Pose pose){
